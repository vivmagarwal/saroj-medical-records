# 2D Echocardiography & Colour Doppler Study — Surat, Maitreya

- **Date performed**: 2026-07-06
- **Category**: ECHO
- **Facility**: Maitreya Multi Superspeciality Hospital & Research Centre, Surat (inpatient, admitted 05-Jul-2026)
- **Referring doctor**: Dr. Sanjay Agrawal · **Cardiologist**: Dr. Narendra Tanwar (DM)
- **Source PDF**: [`../report_pdf/2026-07-06_ECHO_2D-Echo-Colour-Doppler-Surat.pdf`](../report_pdf/2026-07-06_ECHO_2D-Echo-Colour-Doppler-Surat.pdf)
- **Original filename**: page 1 of `Saroj July 2026 Reports_compressed.pdf`

## Summary (extracted)

Context per report: C/o endometrial carcinoma on chemotherapy; H/O pulmonary embolism, S/P mechanical thrombectomy (03/2026).

- **LV**: Mild concentric LVH; no significant RWMA at rest; jerky IVS; **fair LV systolic function, LVEF ~50 %** (RFH echoes: 55 % on 20-Feb, 60 % on 26-Feb and 24-Mar); **diastolic dysfunction grade I**.
- **Valves**: No MS, mild MR; no AS, trivial AR; tricuspid & pulmonary valves normal; **mild-to-moderate TR**.
- **Right heart / pulmonary pressures**: **SEVERE PAH — RVSP/PASP ~70 mmHg**; **mildly dilated RA & RV, normal RV function**; IVC normal (12 mm), good collapse.
- No intracardiac clot / vegetation; no pericardial effusion; intact IAS/IVS; normal aortic root & arch.

**Clinical significance**: Severe pulmonary arterial hypertension **persists**. RVSP/PASP trajectory across the records: **83 mmHg (20-Feb) → 72 (26-Feb) → ~46 (01-Mar, cited only in the discharge-summary narrative — no standalone echo report exists) → 91 (24-Mar, at the PE, with RV systolic dysfunction FAC 20 %) → 70 now** — i.e. much better than the March PE peak, but **higher again than the ~46 mmHg nadir of 01-Mar**, so the PAH may have re-worsened in parallel with the re-accumulating effusions. RV function has recovered vs March (normal now, only mildly dilated RA/RV; IVC normal — no systemic congestion). LVEF ~50 % vs 55–60 % on the RFH echoes — a possible mild drift (within inter-lab variability; worth cardiology follow-up). Note: normal echo does not rule out coronary artery disease (report note).
